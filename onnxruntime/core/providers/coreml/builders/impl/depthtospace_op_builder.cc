...
Status DepthToSpaceOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder,
  ...
  if (model_builder.CreateMLProgram()) {
    ...
    if (mode == "DCR") {
      ...
      AddOperationInput(*reshape1, "x", input_name, model_builder);
      ...
      AddOperationInput(*transpose, "x", reshape1_output, model_builder);
      ...
      AddOperationInput(*reshape2, "x", transpose_output, model_builder);
      AddOperationInput(*reshape2, "shape", model_builder.AddConstant(reshape2->type(), "shape", shape3));

      AddOperationOutput(*reshape2, *output_defs[0], model_builder);
